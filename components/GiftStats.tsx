...
/* Live counter dashboard for one gift campaign. `prefix` namespaces the Redis
   keys ("" = Eid, "newyear:" = Muharram); `label` is shown in the subtitle. */
...
export async function GiftStats({ prefix, label }: { prefix: string; label: string }) {
  ...
  const keys = [
    `${prefix}gift_share:total`,
    ...SHARE_METHODS.map((m) => `${prefix}gift_share:method:${m}`),
    ...CARD_KEYS.map((c) => `${prefix}gift_share:card:${c}`),
    ...SHARE_SOURCES.map((s) => `${prefix}gift_share:source:${s}`),
    `${prefix}claim_free_month:total`,
    ...CARD_KEYS.map((c) => `${prefix}claim_free_month:card:${c}`),
  ];
  const vals = (await redis.mget<(number | null)[]>(...keys)).map(n);
  const m: Record<string, number> = {};
  keys.forEach((k, i) => (m[k] = vals[i]));

  const shareTotal = m[`${prefix}gift_share:total`];
  const claimTotal = m[`${prefix}claim_free_month:total`];
  ...
  const sourceTracked = SHARE_SOURCES.reduce((sum, s) => sum + m[`${prefix}gift_share:source:${s}`], 0);
  ...
  return (
    <main style={{ fontFamily: "system-ui, sans-serif", padding: 40, maxWidth: 680, color: "#192351" }}>
      ...
      <p style={{ color: "#666", marginTop: 0 }}>Live counters · {label}</p>
        ...
        <tbody>
          {SHARE_METHODS.map((mtd) => (
            <tr key={mtd}>
              ...
              <td style={num}>{m[`${prefix}gift_share:method:${mtd}`]}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <table style={{ borderCollapse: "collapse", width: "100%", marginBottom: 28 }}>
        ...
        <tbody>
          {SHARE_SOURCES.map((s) => {
            const v = m[`${prefix}gift_share:source:${s}`];
            return (
              <tr key={s}>
        ...
        <tbody>
          {CARD_KEYS.map((c) => (
            <tr key={c}>
              ...
              <td style={num}>{m[`${prefix}gift_share:card:${c}`]}</td>
              <td style={num}>{m[`${prefix}claim_free_month:card:${c}`]}</td>
            </tr>
          ))}
        </tbody>
      </table>
