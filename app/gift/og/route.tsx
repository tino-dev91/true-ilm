...
type Theme = { bg: string; text: string; accent: string; starOpacity: number };

const THEMES: Record<string, Theme> = {
  lantern: { bg: "#192351", text: "#FEF7E6", accent: "#F5D27A", starOpacity: 0.85 },
  skyline: { bg: "#FFE8B3", text: "#192351", accent: "#B5742A", starOpacity: 0 },
  ornament: { bg: "#FBB1B8", text: "#192351", accent: "#B5495A", starOpacity: 0 },
};
...
export async function GET(req: Request) {
  ...
  /* Generic headline mode (e.g. the homepage thumbnail) — replaces the
     personalised "you have an Eid gift / Open up!" copy. */
  const title = (searchParams.get("title") || "").trim();

  const poppins = await fetch(new URL("./Poppins-Bold.ttf", import.meta.url)).then((r) => r.arrayBuffer());

  const stars: [number, number, number][] = isDark
    ? [
        [120, 90, 5],
        [320, 150, 3],
        [560, 80, 4],
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        {/* kicker */}
        <div style={{ display: "flex", fontSize: 26, letterSpacing: 10, color: theme.accent }}>
          EID MUBARAK
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", maxWidth: 760 }}>
          {title ? (
            <div style={{ display: "flex", fontSize: 64, color: theme.text, lineHeight: 1.08, letterSpacing: -1 }}>
              {title}
            </div>
          ) : null}
          {!title && to ? (
            <div style={{ display: "flex", fontSize: 90, color: theme.text, lineHeight: 1.02, letterSpacing: -2 }}>
              {`${to},`}
            </div>
          ) : null}
          {!title ? (
            <div
              style={{ display: "flex", fontSize: 52, color: theme.text, lineHeight: 1.1, letterSpacing: -1, marginTop: to ? 6 : 0 }}
            >
              you have an Eid gift.
            </div>
          ) : null}
          {!title ? (
            <div style={{ display: "flex", fontSize: 52, color: theme.accent, lineHeight: 1.1, letterSpacing: -1, marginTop: 2 }}>
              Open up!
            </div>
          ) : null}
        </div>
